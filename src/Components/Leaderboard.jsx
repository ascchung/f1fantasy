...
const PodiumPosition = ({ player, position, points }) => {
  ...
  return (
    <div className={`flex flex-col items-center ${positions[position]}`}>
      <div className="text-6xl mb-2">{trophies[position]}</div>
      <div className="text-white text-center mb-2">
        <div className="font-bold text-lg">{player}</div>
        <div className="text-sm opacity-90">{points} pts</div>
      </div>
      <div
        className={`w-24 ${heights[position]} ${colors[position]} rounded-t-lg shadow-lg transform transition-all duration-1000 hover:scale-105 flex items-end justify-center pb-2`}
      >
              ...
              {sorted.map((entry, index) => (
                <tr
                  ...
                  className={`border-t border-gray-600 hover:bg-red-900 hover:bg-opacity-30 text-white transition-all duration-300 ${
                    index < 3
                      ? "bg-gradient-to-r from-yellow-900 to-transparent bg-opacity-20"
                      : ""
                  }`}
                >
